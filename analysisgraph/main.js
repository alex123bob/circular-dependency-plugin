...
async function _links(d3, FileAttachment) {
  return d3.csvParse(await FileAttachment('suits.csv').text());
}
...
function _data(links) {
  return { nodes: Array.from(new Set(links.flatMap(l => [l.source, l.target])), id => ({ id })), links };
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ['suits.csv', { url: new URL('./files/analysis.csv', import.meta.url), mimeType: 'text/csv', toString }],
  ]);
  main.builtin(
    'FileAttachment',
